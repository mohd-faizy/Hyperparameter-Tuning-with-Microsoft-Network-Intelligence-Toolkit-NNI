...
def create_model(num_units, dropout_rate, lr, activation):
    ...
    model.compile(
        ...
        optimizer=tf.keras.optimizers.Adam(learning_rate=lr),  # Corrected parameter name
        metrics=["accuracy"]
    )
    return model
...
class ReportIntermediates(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Explicitly encode the log message to avoid UnicodeEncodeError
        log_message = f"[Epoch {epoch}] Validation accuracy: {acc:.4f}\n"
        sys.stdout.write(log_message)  # Use sys.stdout.write instead of sys.stdout.buffer.write
        sys.stdout.flush()
